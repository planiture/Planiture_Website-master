...
var LCSKChat = function () {
    ...
    function SetDefaults() {
        ...
        options.headerTextColor = 'darkgoldenrod';
        options.headerBorderColor = '#808080';
        options.headerGradientStart = '#000000';
        options.headerGradientEnd = '#000000';
        options.headerFontSize = '15px';

        options.boxBorderColor = '#808080';
        options.boxBackgroundColor = '#000000';
    ...
    function init() {
        $('body').append(
            ...
            '<div id="chat-box-header" style="display: block;position:' + options.position + ';' + getPlacement() + 'width:' + options.width + 'px;padding:' + options.headerPaddings + ';color:' + options.headerTextColor + ';font-size:' + options.headerFontSize + ';cursor:pointer;background:' + options.headerBackgroundColor + ';filter: progid:DXImageTransform.Microsoft.gradient(startColorstr=\'' + options.headerGradientStart + '\', endColorstr=\'' + options.headerGradientEnd + '\');background: -webkit-gradient(linear, left top, left bottom, from(' + options.headerGradientStart + '), to(' + options.headerGradientEnd + '));background: -moz-linear-gradient(top,  ' + options.headerGradientStart + ',  ' + options.headerGradientEnd + ');border:1px solid ' + options.headerBorderColor + ';box-shadow:inset 0 0 7px #0354cb;-webkit-box-shadow:inset 0 0 7px #000000;border-top-left-radius: 5px;border-top-right-radius: 5px;">' + options.offlineTitle + '</div>' +
            '<div id="chat-box" style="display:none;position:' + options.position + ';' + getPlacement() + 'width:' + options.width + 'px;height:300px;padding: 10px 10px 10px 10px;border: 1px solid ' + options.boxBorderColor + ';background-color:' + options.boxBackgroundColor + ';opacity: 0.9;font-size:14px !important;color: white !important;"></div>'
        );
        ...
        connection.start().then(function () {
            ...
            connection.invoke("LogVisit", document.location.href, document.referrer, response.city, response.region, response.country, existingChatId);
        }).catch(function () {
            ...
            keydown: function (e) {
                ...
                if (e.keyCode == 13 && msg != '') {
                    ...
                    if (chatId == null || chatId == '') {
                        connection.invoke("requestChat", msg);
        ...
        $('#chat-box').on({
            click: function () {
                connection.invoke("sendEmail", $('#chat-box-email').val(), $('#chat-box-cmt').val());
    ...
    connection.on("OnlineStatus", function (state) {
    ...
    function ChatRefreshState(state) {
        if (state) {
            ...
            if (!chatEditing) {
                ...
                $('#chat-box-input').hide();
                $('#chat-box').html(
                    '<p>Your email</p><input type="text" id="chat-box-email" style="background-color: black;color:white;border:1px solid darkgoldenrod;border-radius: 3px;width: 100%;" class="chat-editing" />' +
                    '<p>Your message</p><textarea id="chat-box-cmt" cols="40" rows="7" class="chat-editing" style="color: white; background-color: black;border:1px solid darkgoldenrod ;border-radius: 3px;"></textarea>' +
                    '<p><input style="color: white; background-color: black; box-shadow: rgb(128, 128, 128);" type="button" id="chat-box-send" value="Contact us" />'
                );
            }
        }
    }
    ...
    connection.on("SetChat", function (id, agentName, existing) {
        chatId = id;
        requestChat = true;
    ...
    connection.on("OpenChatWindow", function () {
        if (!$('#chat-box').hasClass('chat-open')) {
            toggleChatBox();
        }
    });

    connection.on("AddMessage", function (from, msg) {
        if (chatId != null && chatId != "") {
            if (!requestChat) {
                if (!$('#chat-box').hasClass('chat-open')) {
                    toggleChatBox();
    ...
    connection.on("EmailResult", function (state) {
        if (!state) {
