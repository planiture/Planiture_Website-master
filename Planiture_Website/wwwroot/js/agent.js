...
$(function () {
    ...
    $('#login-btn').click(function () {
        ...
        connection.start()
            .then(function () {
                connection.invoke("agentConnect", agentName, agentPass);
            })
            .catch(function () { alert('unable to connect'); });
    });
    ...
    document.getElementById("change-status").addEventListener("click", function () {
        ...
        connection.invoke("ChangeStatus", agent.isOnline);
        showStatus();
    });
    ...
    $('#current-visits').on({
        click: function (e) {
            ...
            connection.invoke("EngageVisitor", connectionId);
        }
    }, '.engage-visitor');
    ...
    connection.on("NewVisit", function (page, referrer, city, country, chatWith, connectionId) {
        var d = new Date();
    ...
    connection.on("NewChat", function (id) {
        var snd = new Audio("/sounds/newchat.mp3");
        snd.play();
    ...
    connection.on("VisitorSwitchPage", function (lastId, newId, newPage) {
    ...
    connection.on("AddMessage", function (id, from, value) {
        if (id == "internal") {
    ...
    connection.on("Leave", function () {
        connection.invoke("LeaveChat", id);
    });
});
...
function postMsg() {
    ...
    if (msg.substring(0, 1) == '/') {
        ...
        if (chatId != '' && msg != '') {
            connection.invoke("OpSend", chatId, msg);
        }
    }
}
...
function commandTriggered(cmd) {
    ...
    if (parts[0] == 'list') {
        ...
        var chatId = getCurrentChatId();
        var t = (document.getElementById("chatmsgs") + chatId);
        connection.invoke("Transfer", chatId, parts[1], $('#chatmsgs' + chatId).html());
        ...
        if (command != null && chatId != '') {
            connection.invoke("OpSend", chatId, command.msg);
        } else {
